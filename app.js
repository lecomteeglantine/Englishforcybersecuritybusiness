...
document.querySelectorAll("[data-go='diagnostic']").forEach(btn=>btn.addEventListener("click",()=>appNavigate("diagnostic",{push:true})));
...
function tutorSkillLabels(){return{grammar:"Grammar",cyber:"Cyber English",listening:"Listening",pronunciation:"Pronunciation awareness",speaking:"Speaking confidence",writing:"Writing confidence"};}
...
function copyTutorReview(){flushTutorNotes();const fb=document.getElementById("tutorReviewFeedback");if(navigator.clipboard?.writeText)navigator.clipboard.writeText(tutorReviewPlainText()).then(()=>{fb.className="activity-summary correct";fb.textContent="Pedagogical summary copied ✓";}).catch(()=>{fb.className="activity-summary neutral";fb.textContent="Clipboard access was blocked by this browser.";});else{fb.className="activity-summary neutral";fb.textContent="Clipboard access is not available in this browser.";}}
function downloadTutorReview(){flushTutorNotes();const blob=new Blob([tutorReviewPlainText()],{type:"text/plain;charset=utf-8"}),url=URL.createObjectURL(blob),a=document.createElement("a");a.href=url;a.download=`english-back-on-track-tutor-review-${new Date().toISOString().slice(0,10)}.txt`;document.body.appendChild(a);a.click();a.remove();setTimeout(()=>URL.revokeObjectURL(url),1000);const fb=document.getElementById("tutorReviewFeedback");fb.className="activity-summary correct";fb.textContent="Text review downloaded ✓";}
function printTutorReview(){flushTutorNotes();refreshTutorPrintNotes();document.body.classList.add("printing-tutor-review");document.body.classList.toggle("tutor-print-no-notes",!document.getElementById("includeTutorNotesExport")?.checked);const done=()=>{document.body.classList.remove("printing-tutor-review","tutor-print-no-notes");window.removeEventListener("afterprint",done);};window.addEventListener("afterprint",done);window.print();setTimeout(()=>{if(document.body.classList.contains("printing-tutor-review"))done();},2500);}
...
function validateVaultPayload(payload){
  ...
  if(!Number.isFinite(schema)||schema<1)return {ok:false,error:"The backup schema is invalid."};
  if(schema>vaultSchemaVersion)return {ok:false,error:"This backup was created by a newer incompatible version of the app."};
...
function openVaultImportPreview(payload,accepted){
  ...
  document.getElementById("vaultImportPreview").innerHTML=`
    <div class="vault-import-meta">
      <div><span>App version</span><strong>${payload.appVersion||"unknown"}</strong></div>
      <div><span>Backup schema</span><strong>${payload.schemaVersion||"unknown"}</strong></div>
...
function clearVaultGroup(id){
  ...
  if(id==="diagnostic")localStorage.removeItem(diagnosticDraftKey);
  renderProgressVault();
...
function eraseAllVaultData(){
  ...
  [...vaultKnownKeys(),vaultMetaKey,diagnosticDraftKey].forEach(key=>localStorage.removeItem(key));
...
document.querySelectorAll('#my-plan [data-go="diagnostic"]').forEach(btn=>btn.addEventListener('click',()=>appNavigate("diagnostic",{push:true})));
...
function setAppNavCurrent(id){
  ...
  document.querySelectorAll(".nav-group").forEach(el=>{
    const current=el.dataset.navGroup===group;
    el.classList.toggle("current",current);
    const summary=el.querySelector(":scope > summary");
    if(summary){if(current)summary.setAttribute("aria-current","page");else summary.removeAttribute("aria-current");}
  });
}
...
function initV23AppMode(){
  ...
  document.documentElement.classList.remove("app-mode-pending","returning-preload","diagnostic-preload","deep-link-preload");
